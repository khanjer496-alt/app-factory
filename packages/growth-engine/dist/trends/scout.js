export class TrendScout {
    ...
    async run(product, options = {}) {
        ...
        const sanitized = raw.map(sanitizeSignal).filter((s) => Boolean(s));
...
export function scoreTrend(signal, product, now = new Date()) {
    ...
    const trendScore = Math.round(100 * clamp(relevance * 0.36 +
        freshness * 0.18 +
        velocity * 0.18 +
        volume * 0.10 +
        engagement * 0.06 +
    ...
    const reasons = [];
    if (relevance >= 0.65)
        reasons.push("strong product/audience relevance");
    if (freshness >= 0.8)
        reasons.push("fresh signal");
    if (velocity >= 0.65)
        reasons.push("high observed velocity");
    if (volume >= 0.65)
        reasons.push("large observed volume");
    if (engagement >= 0.65)
        reasons.push("strong engagement");
    if (!reasons.length)
        reasons.push("moderate multi-signal fit");
    return { ...signal, trendScore, productRelevance: relevance, freshness, reasons };
}
export function deriveProductKeywords(product) {
    return unique([
        product.name,
        product.oneLiner,
        ...product.audiences,
...
function productRelevance(signal, product) {
...
function freshnessScore(signal, now) {
...
function normalizeMagnitude(value, reference) {
...
function sanitizeSignal(signal) {
    ...
    const relatedTerms = signal.relatedTerms
        ...
        : undefined;
    const cleaned = { ...signal, text, keywords };
    if (url)
        cleaned.url = url;
    else
        delete cleaned.url;
    if (relatedTerms)
        cleaned.relatedTerms = relatedTerms;
    else
        delete cleaned.relatedTerms;
    return cleaned;
}
...
function diversifyBySource(signals, maxSelected, maxPerSource) {
...
function toErrorMessage(value) { return value instanceof Error ? value.message : String(value); }
