...
import { createCloudflareGrowthRuntime } from "./runtime";
import { audit, finishDailyGrowthRun, loadGrowthProducts, loadLearnings, loadRecentPerformance, loadSocialConnections, loadTodayUsage, persistApproval, persistLearnings, persistMetricObservations, persistPublication, persistQAResult, persistRenderResult, recordSpend, startDailyGrowthRun, } from "../storage/daily";
/**
 ...
 */
export class DailyGrowthWorkflow extends WorkflowEntrypoint {
    async run(event, step) {
        const db = this.env.GROWTH_DB;
    ...
    async runProduct(product, event, step, maxIdeas) {
        const db = this.env.GROWTH_DB;
        ...
        try {
            ...
            const plan = await step.do(`${safeName}: plan campaign and QA`, async () => {
                ...
                for (const row of qaRows)
                    await persistQAResult(db, product.productId, row.idea.id, row.qa);
                return qaRows;
            });
            ...
            for (const row of plan) {
                ...
                if (idea.format !== "text_post") {
                    ...
                    const renderRequest = { id: `rr_${idea.id}`, productId: product.productId, idea, aspectRatio: "9:16", outputR2Prefix: `growth/${product.productId}/${idea.id}` };
                    ...
                    if ((renderResult.status === "queued" || renderResult.status === "rendering") && renderer.status && renderResult.providerJobId) {
                        ...
                        for (let poll = 1; poll <= 12; poll++) {
                            ...
                            if (renderResult.status === "completed" || renderResult.status === "failed")
                                break;
                        }
                    }
            ...
            const status = reviews > 0 ? "partial" : "completed";
...
//# sourceMappingURL=daily-workflow.js.map