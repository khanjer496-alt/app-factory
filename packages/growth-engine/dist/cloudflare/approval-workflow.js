...
import { audit, decideApproval, loadApprovalBundle, loadSocialConnections, loadTodayUsage, persistPublication, } from "../storage/daily";
/** Publish a previously reviewed item after a human approval event. */
export class ApprovalPublishWorkflow extends WorkflowEntrypoint {
    async run(event, step) {
        const db = this.env.GROWTH_DB;
        const runtime = createCloudflareGrowthRuntime(this.env);
        const payload = event.payload;
        if (!payload?.approvalId || !payload?.approvedBy)
            throw new Error("approvalId and approvedBy are required");
        const bundle = await step.do("load approval", () => loadApprovalBundle(db, payload.approvalId));
        if (!bundle)
            throw new Error("Approval not found");
        if (bundle.status !== "pending")
            return { status: "ignored", reason: `Approval is ${bundle.status}` };
