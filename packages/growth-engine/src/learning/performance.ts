...
export function derivePerformanceLearnings(rows: ObservedContentPerformance[]): PerformanceLearning[] {
  if (!rows.length) return [];
  const buckets = new Map<string, { revenue: number; conversions: number; clicks: number; count: number }>();
