...
import { scoreIdea, type IdeaSignals, rankIdeas } from "../scoring";
...
export class ContentBrain {
  async generate(
    ...
    options: ContentBrainOptions = {},
  ): Promise<ContentBrainReport> {
  ...
  private parseModelIdeas(text: string, product: ProductBrain, trends: ScoredTrendSignal[], count: number): ContentIdea[] {
    const json = extractJson(text);
    const parsed = JSON.parse(json) as { ideas?: ModelIdea[] };
    const knownTrendIds = new Set(trends.map((t) => t.id));
    return (parsed.ideas ?? []).slice(0, count).flatMap((raw, index) => {
      if (!raw.hook?.trim() || !raw.angle?.trim()) return [];
...
function fillSignals(partial: Partial<IdeaSignals> | undefined, trendIds: string[] | undefined, trends: ScoredTrendSignal[]): IdeaSignals {
  const linked = trends.filter((t) => trendIds?.includes(t.id));
  const mean = (getter: (t: ScoredTrendSignal) => number, fallback: number) => linked.length
...
function validFormat(value: unknown): value is ContentFormat {
  return ["faceless_video", "slideshow", "product_demo", "ugc_avatar", "image_post", "text_post"].includes(String(value));
}
function mutateHook(hook: string, index: number): string {
...
function rotateFormat(format: ContentFormat, step: number): ContentFormat {
  const formats: ContentFormat[] = ["product_demo", "slideshow", "faceless_video", "ugc_avatar", "text_post"];
