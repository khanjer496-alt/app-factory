import type { ContentFormat, ContentIdea, ProductBrain, ScoredTrendSignal } from "../types";

const formatOrder: ContentFormat[] = ["product_demo", "slideshow", "faceless_video", "ugc_avatar", "text_post"];

export function deterministicIdeas(
...
function chooseFormat(trend: ScoredTrendSignal, index: number): ContentFormat {
  if (trend.format?.includes("photo") || trend.format?.includes("carousel")) return "slideshow";
  if (trend.format?.includes("text")) return "text_post";
  return formatOrder[index % formatOrder.length] ?? "faceless_video";
}
function hooks(product: ProductBrain, trend: ScoredTrendSignal): string[] {
  return [
...
function angles(product: ProductBrain): string[] {
  return [
...
function buildScript(product: ProductBrain, trend: ScoredTrendSignal, hook: string, angle: string, cta: string): string {
...
function visualPlan(product: ProductBrain, format: ContentFormat): string[] {
  if (format === "product_demo") return ["Hook text", "Open product demo", "Show one core action", "Show result", `CTA: ${product.domain}`];
  if (format === "slideshow") return ["Bold hook slide", "Problem slide", "Insight slide", "Product proof slide", "CTA slide"];
  if (format === "ugc_avatar") return ["Natural talking-head hook", "Problem story", "Cut to product demo", "Result", "Soft CTA"];
  return ["Hook", "Relevant B-roll/product asset", "Proof", "Result", "CTA"];
}
function syntheticTrend(product: ProductBrain): ScoredTrendSignal {
  return {
