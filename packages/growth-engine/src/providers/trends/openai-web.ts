 ...
 */
export class OpenAIWebTrendProvider implements TrendProvider {
  readonly name = "openai_web_search";
  private readonly fetcher: typeof fetch;
  constructor(private readonly options: OpenAIWebTrendProviderOptions) {
  ...
  async collect(query: TrendQuery): Promise<TrendSignal[]> {
    ...
    const text = payload.output_text ?? payload.output
      ...
      .map((item) => item.text ?? "")
      .join("") ?? "";
    const parsed = JSON.parse(extractJson(text)) as { signals?: WebTrendItem[] };
