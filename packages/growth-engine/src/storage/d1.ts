import type { ContentBrainReport, ScoredTrendSignal, TrendScoutReport } from "../types";
...
export async function persistTrendScoutReport(db: D1DatabaseLike, report: TrendScoutReport): Promise<void> {
...
export async function persistContentBrainReport(db: D1DatabaseLike, report: ContentBrainReport): Promise<void> {
  const statements: D1PreparedStatementLike[] = [
  ...
  for (const idea of report.ideas) {
    statements.push(db.prepare(`INSERT OR REPLACE INTO content_ideas
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?)`)
      .bind(
        idea.id, idea.productId, JSON.stringify(idea.sourceTrendIds), idea.hook, idea.angle, idea.format,
        idea.language, idea.script ?? null, JSON.stringify(idea.visualPlan ?? []), idea.cta ?? null,
        idea.score, idea.confidence, idea.risk, report.generatedAt,
      ));
  }
...
function upsertTrendStatement(db: D1DatabaseLike, trendProductId: string, trend: ScoredTrendSignal): D1PreparedStatementLike {
  return db.prepare(`INSERT OR REPLACE INTO trend_signals
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(
      trend.id, trendProductId, trend.source, trend.observedAt, trend.region ?? null, trend.language ?? null,
      trend.format ?? null, trend.text, trend.url ?? null, JSON.stringify(trend.keywords), trend.velocity ?? null,
      trend.confidence, trend.expiresAt ?? null, JSON.stringify({
