...
function Legal({ kind }: { kind: "privacy" | "terms" }) {
  return (
    <PageShell className="prose">
      <span className="eyebrow"><i className="dot" />Template · replace before launch</span>
      <h1 className="display l">{productConfig.name} {kind === "privacy" ? "privacy policy" : "terms of service"}</h1>
      {kind === "privacy"
        ...
        : <p>Cover eligibility, plan purchase and refunds, delivery windows and failed deliveries, the 48-hour change lock, skipped days, allergen disclaimer, acceptable use, liability and governing law (UAE).</p>}
    </PageShell>
  );
}
